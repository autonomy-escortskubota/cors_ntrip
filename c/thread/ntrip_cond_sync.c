...
int buff[BUF_S];
int cout = 0; // Number of items in buffer
int add = 0;   // Next place to add an item
int rem = 0;   // Next place to remove an item
...
pthread_cond_t cond_producer = PTHREAD_COND_INITIALIZER; // for producers to wait (buffer full)
pthread_cond_t cond_consumer = PTHREAD_COND_INITIALIZER; // for consumers to wait (buffer empty)
...
void *serial_reader_thread(void *arg) {
    ...
    while ((count = read(serial_fd, serial_buffer, sizeof(serial_buffer) - 1))>0) {
